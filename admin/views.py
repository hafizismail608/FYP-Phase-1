...
def admin_required(f):
    ...
    def decorated_function(*args, **kwargs):
        if not current_user.is_authenticated or (current_user.role != 'admin' and current_user.role != 'super_admin'):
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user and (user.role == 'admin' or user.role == 'super_admin') and check_password_hash(user.password_hash, form.password.data):
            login_user(user)
...
def create_user():
    ...
    if form.validate_on_submit():
        ...
        # Create new user
        is_protected = form.role.data == 'super_admin'
        user = User(
        ...
        if form.role.data == 'student' or form.role.data == 'admin' or form.role.data == 'super_admin':
...
def edit_user(user_id):
    ...
    # Prevent editing of super_admin role for protected users
    if user.role == 'super_admin' and user.is_protected and current_user.id != user.id:
        flash('Super admin users cannot be edited.', 'danger')
        return redirect(url_for('admin.users'))
    ...
    if form.validate_on_submit():
        ...
        # Update profile
        if user.role == 'student' or user.role == 'admin' or user.role == 'super_admin':
...
def delete_user(user_id):
    ...
    # Prevent deletion of super_admin or protected users
    if user.role == 'super_admin' or user.is_protected:
...
def toggle_user_status(user_id):
    ...
    # Prevent deactivation of super_admin or protected users
    if user.role == 'super_admin' or user.is_protected:
        flash('This user cannot be deactivated.', 'danger')
        return redirect(url_for('admin.users'))
